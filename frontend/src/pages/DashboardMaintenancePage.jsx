...
export default function DashboardMaintenancePage() {
  ...
  useEffect(() => {
    planningAPI.listerLignes().then(r => setLignes(r.data)).catch(() => {});
  }, []);
  ...
  const parMaint = synthese?.par_maintenancier || [];
  ...
  const evolution = graphs?.evolution || [];
  const hasEvolution = evolution.some(e => e.heures_correctives > 0 || e.nb_interventions > 0);
  const dispoLignes = graphs?.dispo_par_ligne || [];
  const repartition = graphs?.repartition || [];
  const hasRepartition = repartition.some(r => r.value > 0);
  const parSemaine = graphs?.par_semaine || [];
    ...
    <div className="mx-auto max-w-7xl space-y-6 p-4 md:p-6">
      ...
      <div className="grid grid-cols-1 gap-6 xl:grid-cols-2">
        ...
        <ChartCard
          ...
          empty={parSemaine.every(s => !s.heures)}
        >
          <ResponsiveContainer width="100%" height="100%">
      ...
      {parLigne.map(groupe => (
        <section key={groupe.ligne_code} className="overflow-hidden rounded-2xl border border-border bg-card shadow-sm">
          <div className="flex items-center gap-3 border-b border-border bg-gradient-to-r from-primary/5 to-transparent px-5 py-4">
            ...
            <h2 className="font-semibold">{groupe.ligne_nom || 'Ligne'}</h2>
          </div>
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              <thead>
